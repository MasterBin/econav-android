    ...
    init {
                ...
                when(val state = gstate.routingState) {
                    is MainStore.State.Routing.CreateRouteByPlacingPoints -> {
                        state.startPoint.ifNotNull {
                            overlayPoints.addItem(OverlayItem(null, null, it.toGeoPoint()))
                        }
                    ...
                    is MainStore.State.Routing.RouteCreated -> {
                        state.route.also {
                            overlayPoints.addItem(OverlayItem(null, null,  it.from.toGeoPoint()))
                            overlayPoints.addItem(OverlayItem(null, null,  it.to.toGeoPoint()))
        ...
        componentScope.launch {
            store.labels.collect {
                when(it) {
                    is MainStore.Label.RequestRoute -> requestRoute(it.startPoint, it.endPoint)
                }
            }
        }
    }

    private fun requestRoute(from : LatLon, to : LatLon) {
